...
const MangaPanel = ({ panel, panelIndex, sceneIndex, editMode, onEdit }) => {
      ...
      const patterns = {
        ...
        'square+zigzag': (
          <g>
            ...
            />
            <path
              d="M 18 32 L 22 28 L 26 36 L 30 28 L 32 32"
              stroke="#000"
              strokeWidth="2"
              fill="none"
            />
  ...
  return (
    ...
    >
      {/* パネル番号表示 */}
      <div className="panel-number-badge">{panel.panel_number}</div>
      ...
      {panel.content.narration && (
        <div className="panel-narration">
          <span className="narration-text">{panel.content.narration}</span>
        </div>
      )}
      ...
      {/* 編集モード時の操作ヒント */}
      {editMode && <div className="edit-hint">✏️ クリックして編集</div>}

      {/* 視覚的注意点 */}
      {panel.visual_notes && (
        <div className="visual-notes">{panel.visual_notes}</div>
      )}
    </div>
  );
};
